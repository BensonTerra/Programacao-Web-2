...
const server = http.createServer((request, response) => {
    ...
    if (parsedURL.pathname == '/authors') {
        ...
        if (request.method == 'GET') {
            ...
            connection.query('SELECT * FROM authors', (err, results) => {
                if (err) {
                    ...
                    response.writeHead(DATABASE_ERROR.statusCode, { 'Content-Type': 'application/json' });
            ...
            request.on('end', () => {
                ...
                if (newAuthor.name == undefined || newAuthor.city == undefined) {
                    //set response for DB acess error
                    response.writeHead(NO_BODY_DATA.statusCode, { 'Content-Type': 'application/json' });
            ...
            if (ID == null) { //set response for bad request
                response.writeHead(INVALID_FILTER.statusCode, { 'Content-Type': 'application/json' });
            ...
            request.on('end', () => {
                ...
                connection.query('UPDATE authors SET name = ?, city = ? WHERE id = ?',
                    ...
                    function (err, results) {
                        ...
                        if (results.affectedRows == 1) { // DB responds with # of authors affected by UPDATE
                            response.statusCode = 204;
                            response.end();
                            return;
                        }
                        else { //set response for AUTHOR not found
                            response.writeHead(AUTHOR_NOT_FOUND.statusCode, { 'Content-Type': 'application/json' });
            ...
            //query the BD with an INSERT statementâ€‹
            connection.query('DELETE FROM authors WHERE id = ?', [ID], function (err, results) {
                if (err) {
                    //set response for DB acess error
                    response.writeHead(DATABASE_ERROR.statusCode, { 'Content-Type': 'application/json' });
                    response.end(JSON.stringify(DATABASE_ERROR.message));
        ...
        else { //set response for invalid VERB
            response.writeHead(INVALID_VERB.statusCode, { 'Content-Type': 'application/json' });
            response.end(JSON.stringify(INVALID_VERB.message));
        }
    } else { //set response for invalid other invalid requests
        response.writeHead(RESOURCE_NOT_FOUND.statusCode, { 'Content-Type': 'application/json' });
